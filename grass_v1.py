...
while True:
    ...
    for tile in my_units:
        ...
        if tile.units > 1:
            target = Target(tile.x +1,tile.y +1)
        else:
            distance = [math.dist([tile.x,tile.y],[t.x,t.y]) for t in my_units]
            tuile_proche = my_units[distance.index(sorted(distance)[1])]
            target = Target(tuile_proche.x,tuile_proche.y)
        print(f"Debug messages... {target}", file=sys.stderr, flush=True)
        if target:
